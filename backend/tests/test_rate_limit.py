...
from app.services.rate_limit import FixedWindowRateLimiter, SharedFixedWindowRateLimiter
...
def test_shared_rate_limiter_blocks_after_max_attempts_and_resets() -> None:
    current_time = datetime(2026, 3, 14, tzinfo=timezone.utc)

    def now_provider() -> datetime:
        return current_time

    limiter = SharedFixedWindowRateLimiter(
        max_attempts=2,
        window_seconds=60,
        store=CacheStore(backend=MemoryCacheBackend(now_provider=now_provider), namespace="test"),
        namespace="auth",
    )
